...
@section('content')
    <div class="max-w-2xl">
        <div class="flex items-center gap-4 mb-8">
            ...
            <h1 class="text-2xl font-bold text-gray-800">Edit Kios: {{ $kiosk->name }}</h1>
        </div>

        <form method="POST" action="{{ route('admin.kiosks.update', $kiosk) }}" enctype="multipart/form-data"
            class="card p-6 space-y-6">
            @csrf @method('PUT')
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nama Kios</label>
                <input type="text" id="name" name="name" value="{{ old('name', $kiosk->name) }}" class="input-field"
                    required>
                ...
                <div>
                    ...
                    <select name="kantin_id" id="kantin_id" class="input-field" required>
                        ...
                        @foreach($kantins as $kantin)
                            <option value="{{ $kantin->id }}" {{ old('kantin_id', $kiosk->kantin_id) == $kantin->id ? 'selected' : '' }}>{{ $kantin->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="user_id" id="user_id" class="input-field" required>
                        ...
                        @foreach($penjuals as $penjual)
                            <option value="{{ $penjual->id }}" {{ old('user_id', $kiosk->user_id) == $penjual->id ? 'selected' : '' }}>{{ $penjual->name }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                    class="input-field">{{ old('description', $kiosk->description) }}</textarea>
